...
class Database:
    ...
    def _adapt_sql_for_sqlite(self, sql: str) -> str:
        ...
        # SQLite does not support MySQL/MariaDB's conditional ADD COLUMN
        # syntax. Migration statements are already applied individually and
        # duplicate-column errors are safely logged, so remove the conditional
        # words while retaining the column addition itself.
        sql = re.sub(
