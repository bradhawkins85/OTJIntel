"""Smoke tests for the ``ollama`` feature pack."""
...
from app.features.ollama import PACK
...
def test_ollama_pack_manifest_declares_all_routes():
    declared = set()
    ...
    assert PACK.slug == "ollama"
    assert PACK.version
    assert declared == EXPECTED


def test_ollama_pack_is_enabled_by_default():
...
def test_ollama_pack_loads_and_reloads_cleanly():
    async def _run() -> None:
        test_app = FastAPI()
        registry = init_registry(test_app)

        await registry.load("ollama")
        after_load = _routes_for(test_app)
        assert EXPECTED.issubset(after_load)

        await registry.reload("ollama")
