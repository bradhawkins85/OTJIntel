"""Smoke tests for the ``notifications`` feature pack."""
...
from app.features.notifications import PACK
...
def test_notifications_pack_manifest_declares_all_routes():
    ...
    assert PACK.slug == "notifications"
    assert PACK.version
    assert declared == EXPECTED


def test_app_main_no_longer_owns_notifications_routes():
...
def test_notifications_pack_loads_and_reloads_cleanly():
    ...
    async def _run() -> None:
        ...
        await registry.load("notifications")
        after_load = _routes_for(test_app)
        assert EXPECTED.issubset(after_load)

        await registry.reload("notifications")
