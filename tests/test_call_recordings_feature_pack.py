"""Smoke tests for the ``call_recordings`` feature pack."""
...
from app.features.call_recordings import PACK
...
def test_call_recordings_pack_manifest_declares_all_routes():
    declared = set()
    ...
    assert PACK.slug == "call_recordings"
    assert PACK.version
    assert declared == EXPECTED


def test_app_main_no_longer_owns_call_recordings_routes():
...
def test_call_recordings_pack_loads_and_reloads_cleanly():
    import asyncio
    ...
    async def _run() -> None:
        ...
        await registry.load("call_recordings")
        after_load = _routes_for(test_app)
        assert EXPECTED.issubset(after_load)

        await registry.reload("call_recordings")
