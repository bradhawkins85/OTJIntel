"""Smoke tests for the ``subscriptions`` feature pack."""
...
from app.features.subscriptions import PACK
...
def test_subscriptions_pack_manifest_declares_all_routes():
    declared = set()
    ...
    assert PACK.slug == "subscriptions"
    assert PACK.version
    assert declared == EXPECTED


def test_app_main_no_longer_owns_subscriptions_routes():
...
def test_subscriptions_pack_loads_and_reloads_cleanly():
    import asyncio
    ...
    async def _run() -> None:
        ...
        await registry.load("subscriptions")
        after_load = _routes_for(test_app)
        assert EXPECTED.issubset(after_load)

        await registry.reload("subscriptions")
