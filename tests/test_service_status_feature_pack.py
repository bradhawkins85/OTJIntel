"""Smoke tests for the ``service_status`` feature pack."""
...
from app.features.service_status import PACK
...
def test_service_status_pack_manifest_declares_all_routes():
    ...
    assert PACK.slug == "service_status"
    assert PACK.version
    assert declared == EXPECTED


def test_app_main_no_longer_owns_service_status_routes():
...
def test_service_status_pack_loads_and_reloads_cleanly():
    ...
    async def _run() -> None:
        ...
        await registry.load("service_status")
        after_load = _routes_for(test_app)
        assert EXPECTED.issubset(after_load)

        await registry.reload("service_status")
