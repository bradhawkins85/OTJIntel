"""Test edge cases for Xero line item description template."""
import pytest
...
async def test_empty_template_uses_default():
    """Test that an empty template string falls back to default."""
    ...
    module_settings = {
        ...
        "settings": {
            ...
            "line_item_description_template": "",  # Empty template
        },
    }
    
    company = {"id": 1, "name": "Test Company", "xero_id": "xero-123"}
    tickets = [{"id": 42, "status": "billable", "company_id": 1, "subject": "Test Issue"}]
    replies = [{
        "id": 100,
        "minutes_spent": 60,
        "is_billable": True,
    ...
    mock_response.text = json.dumps({"Invoices": [{"InvoiceNumber": "INV-TEST", "Status": "DRAFT"}]})
...
@pytest.mark.anyio("asyncio")
async def test_template_with_invalid_placeholder():
    """Test that template handles invalid placeholders gracefully."""
    
    template = "Ticket {ticket_id}: {invalid_placeholder} - {ticket_subject}"
    ...
    module_settings = {
        ...
        "settings": {
            ...
            "default_hourly_rate": "100.00",
            "billable_statuses": "billable",
            "line_item_description_template": template,
        },
    }
    ...
    with patch("app.services.xero.modules_service") as mock_modules, \
        ...
        # Invalid placeholder should be replaced with empty string (due to _TemplateValues.__missing__)
        assert line_items[0]["Description"] == "Ticket 42:  - Test Issue"
