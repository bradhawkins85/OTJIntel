"""Smoke tests for the ``uptimekuma`` feature pack."""
...
from app.features.uptimekuma import PACK
...
def test_uptimekuma_pack_manifest_declares_all_routes():
    declared = set()
    ...
    assert PACK.slug == "uptimekuma"
    assert PACK.version
    assert declared == EXPECTED


def test_uptimekuma_pack_routes_are_implemented_in_feature_pack():
    for router in PACK.routers:
        for route in router.routes:
            assert route.endpoint.__module__.startswith("app.features.uptimekuma"), (
                f"Route {route.path} is implemented in {route.endpoint.__module__}; "
                "uptimekuma feature-pack code should live under app.features.uptimekuma."
            )


def test_uptimekuma_pack_is_enabled_by_default():
    default_feature_packs = str(Settings.model_fields["feature_packs"].default).split(",")
    assert "uptimekuma" in default_feature_packs

    env_example = (PROJECT_ROOT / ".env.example").read_text(encoding="utf-8")
    assert "FEATURE_PACKS=" not in env_example


def test_app_main_no_longer_owns_uptimekuma_routes():
...
def test_uptimekuma_pack_loads_and_reloads_cleanly():
    async def _run() -> None:
        test_app = FastAPI()
        registry = init_registry(test_app)

        await registry.load("uptimekuma")
        after_load = _routes_for(test_app)
        assert EXPECTED.issubset(after_load)

        await registry.reload("uptimekuma")
